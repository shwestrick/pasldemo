...
template <class Pred>
class pack_contr_type {
public:
  static loop_controller_type flags;
  static loop_controller_type psums;
  static loop_controller_type pack;
};
template <class Pred>
loop_controller_type pack_contr_type<Pred>::flags("pack_flags" + par::string_of_template_arg<Pred>());
template <class Pred>
loop_controller_type pack_contr_type<Pred>::psums("pack_psums" + par::string_of_template_arg<Pred>());
template <class Pred>
loop_controller_type pack_contr_type<Pred>::pack("pack_pack" + par::string_of_template_arg<Pred>());

template <class Pred>
sparray pack_by_predicate(const Pred& p, const sparray& xs, const long k) {
  long n = xs.size();

  long nb = 1 + (n-1) / k;
  sparray psums(nb);
  par::parallel_for(pack_contr_type<Pred>::psums, 0L, nb, [&] (long i) {
    int count = 0;
    long upper = std::min((i+1)*k, n);
    for (long j = i*k; j < upper; j++) if (p(xs[j])) count++;
    psums[i] = count;
  });
  scan_excl_result offsets = prefix_sums_excl(psums);

  long m = offsets.total;
  sparray result(m);
  par::parallel_for(pack_contr_type<Pred>::pack, 0L, nb, [&] (long i) {
    long offset = offsets.partials[i];
    long upper = std::min((i+1)*k, n);
    for (long j = i*k; j < upper; j++)
      if (p(xs[j])) { result[offset] = xs[j]; offset++; }
  });
  return result;
}
...
sparray filter(const Pred& p, const sparray& xs) {
  return pack_by_predicate(p, xs, 1024L); // block size of 1024
}

/***********************************************************************/

loop_controller_type filter_simple_contr("filter_simple_contr");

template <class Pred>
sparray filter_simple(const Pred& p, const sparray& xs) {
  long n = xs.size();

  scan_excl_result offsets =
    scan_excl([&] (value_type x, value_type y) { return x + y; },
              p, 0, xs);
  // offsets.partials
  // offsets.total

  sparray result = sparray(offsets.total);
  par::parallel_for(filter_simple_contr, (long)0, n, [&] (long i) {
    long curr = offsets.partials[i];
    long next = (i+1 == n) ? offsets.total : offsets.partials[i+1];
